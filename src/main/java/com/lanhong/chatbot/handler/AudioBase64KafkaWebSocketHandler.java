...
@Component
public class AudioBase64KafkaWebSocketHandler implements WebSocketHandler {

    private final Logger logger = LoggerFactory.getLogger(AudioBase64KafkaWebSocketHandler.class);
        ...
        Mono<Void> output = sink.asFlux()
                ...
                .doOnNext(record -> {
                    ...
                    if (targetSession != null) {
                        ...
                        ObjectMapper objectMapper = new ObjectMapper();
                        String audioBase64 = azureTextToSpeech.getAudioBase64(record);
                        ResultEntity resultEntity = new ResultEntity();
                        resultEntity.setAudioBase64(audioBase64);
                        resultEntity.setText(record);
                        try {
                            String resultJson = objectMapper.writeValueAsString(resultEntity);
                            targetSession.send(Mono.just(targetSession.textMessage(resultJson))).subscribe();
                        } catch (JsonProcessingException e) {
                            throw new RuntimeException(e);
                        }
                    }
